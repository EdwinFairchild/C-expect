...
# Target under test
export CMSIS_DAP_ID_1=04091702f7f18a2900000000000000000000000097969906
export TARGET_1_LC=max32655
export TARGET_1_UC=MAX32655
export devSerial_1="$(ls -la /dev/serial/by-id | grep -n 'D309ZDFB'| rev| cut -b 1-7| rev)"
...
export devSerial_2="$(ls -la /dev/serial/by-id | grep -n 'D3073ICQ'| rev| cut -b 1-7| rev)"
...
{
    openocd -f interface/cmsis-dap.cfg -f target/$TARGET_1_CFG -s tcl -c "cmsis_dap_serial  ${CMSIS_DAP_ID_1}" -c "gdb_port 3333" -c "telnet_port 4444" -c "tcl_port 6666"  &
    openocd_dapLink1_pid=$!
}
function start_openocd_DAPLINK_2()
{
    openocd  -f interface/cmsis-dap.cfg -f target/$TARGET_2_CFG -s tcl  -c "cmsis_dap_serial  ${CMSIS_DAP_ID_2}" -c "gdb_port 3334" -c "telnet_port 4445" -c "tcl_port 6665" &
